...
export function buildOrchestrator(agents: Agent[], options: OrchestratorOptions = {}) {
  ...
  fastify.get("/metrics", async (_request, reply) => {
    ...
    const body = [
      ...
      ...llmMetrics.buckets.map(
        (bucket, idx) => `orchestrator_llm_latency_ms_bucket{le="${bucket}"} ${llmMetrics.bucketCounts[idx]}`
      ),
  ...
  fastify.post("/tasks", async (request, reply) => {
    ...
    const requesterId =
      (body?.payload as Record<string, unknown> | undefined)?.["profileId"] ||
      (body?.payload as Record<string, unknown> | undefined)?.["userId"] ||
      (body?.payload as Record<string, unknown> | undefined)?.["identityId"] ||
      headerValue;

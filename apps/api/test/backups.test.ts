...
describe('profile backups and import', () => {
  ...
  it('backs up and restores a profile', async () => {
    ...
    expect(createExpRes.statusCode).toBe(
      200,
      `Failed to create experience: ${JSON.stringify(createExpRes.json())}`,
    );

    const beforeBackup = await server.inject({
      method: 'GET',
      url: `/profiles/${profileId}/experiences`,
    });
    ...
    const checkSnapshot = await server.inject({
      method: 'GET',
      url: `/profiles/${profileId}/backup/${snapshotId}`,
    });
    ...
    await server.inject({
      method: 'DELETE',
      url: `/profiles/${profileId}/experiences/${experienceId}`,
    });
    ...
    const afterRestore = await server.inject({
      method: 'GET',
      url: `/profiles/${profileId}/experiences`,
    });
    expect(afterRestore.json().data).toHaveLength(
      1,
      `Expected 1 experience after restore, got ${afterRestore.json().data.length}`,
    );
  });
});

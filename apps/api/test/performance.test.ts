...
const timeRequest = async (options: any) => {
...
describe("API performance budgets", () => {
  ...
  it("exports JSON-LD within budget", async () => {
    const { res, elapsed } = await timeRequest({ method: "GET", url: `/profiles/${profileId}/export/jsonld` });
    expect(res.statusCode).toBe(200);
    expect(elapsed).toBeLessThan(200);
  });

  it("exports PDF within budget", async () => {
    const { res, elapsed } = await timeRequest({ method: "GET", url: `/profiles/${profileId}/export/pdf` });
    expect(res.statusCode).toBe(200);

...
export class LocalFilesystemProvider implements CloudStorageProvider {
  ...
  private async *listLocalFiles(folderPath: string, options?: ListOptions): AsyncIterable<CloudFile> {
  ...
  private async *listSftpFiles(folderPath: string, options?: ListOptions): AsyncIterable<CloudFile> {
      ...
      try {
          ...
          const traverseSftp = async function* (this: LocalFilesystemProvider, dir: string, relDir: string, depth: number): AsyncIterable<CloudFile> {
             ...
             for (const item of list) {
                 ...
                 if ((item.attrs.mode & 0o170000) === 0o040000) {
                     ...
                     if (options?.filters?.maxFileSize && cloudFile.size > options.filters.maxFileSize) continue;
  ...
  private async downloadSftpFile(fileId: string, destPath: string, onProgress?: (bytes: number) => void): Promise<void> {
      const { Client } = await import("ssh2");
      const url = new URL(this.basePath);
      const rootPath = url.pathname || ".";
      ...
      try {
          ...
          await new Promise<void>((resolve, reject) => {
              sftp.fastGet(targetPath, destPath, {
  ...
  private async *listSmbFiles(folderPath: string, options?: ListOptions): AsyncIterable<CloudFile> {
      ...
      // Recursive traversal
      const traverseSmb = async function* (this: LocalFilesystemProvider, dir: string, relDir: string, depth: number): AsyncIterable<CloudFile> {
          if (depth > (options?.recursive ? (this.depth || 10) : 0)) return;
              ...
              try {
                  ...
                  if (stat.isDirectory()) {
                       ...
                       const cloudFile: CloudFile = {
                           ...
                           name: name as string,
                           path: relPath,
                           mimeType: this.guessMimeType((name as string).split('.').pop() || ""),
                           size: stat.size,
                  ...
                  console.warn(`SMB Stat error for ${fullPath}: ${e}`);
              }
          }
      };
      
      // Normalize folderPath to backslashes
      const startDir = rootDir ? (folderPath ? rootDir + "\\\\" + folderPath.replace(/\//g, '\\\\') : rootDir) : (folderPath ? folderPath.replace(/\//g, '\\\\') : "");
      
      yield* traverseSmb.call(this, startDir, "", 0);
  }

  private async downloadSmbFile(fileId: string, destPath: string, onProgress?: (bytes: number) => void): Promise<void> {
      // @ts-ignore
      const SMB2 = (await import("smb2")).default;
      const url = new URL(this.basePath);
      const parts = url.pathname.split('/').filter(p => p);
      ...
      const smbPath = rootDir ? rootDir + "\\\\" + fileId.replace(/\//g, '\\\\') : fileId.replace(/\//g, '\\\\');
